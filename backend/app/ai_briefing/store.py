...
def _parse_rejections(text: str) -> list[BriefingRejectionRecord]:
    ...
    for line_number, line in enumerate(text.splitlines(), start=1):
        ...
        if record.sequence != line_number or record.previous_hash != previous:
            raise InsightStoreError(
                f"AI rejection audit chain mismatch at line {line_number}."
            )
        ...
        if record.rejection_id != expected_id:
            raise InsightStoreError(
                f"AI rejection audit ID mismatch at line {line_number}."
            )
        payload = record.model_dump(mode="json")
        record_hash = payload.pop("record_hash")
        if _hash(payload) != record_hash:
            raise InsightStoreError(
                f"AI rejection audit hash mismatch at line {line_number}."
            )
        records.append(record)
...
def append_rejection(
    ...
    *,
    ...
    idempotency_fingerprint = _hash({"idempotency_key": idempotency_key})
    rejection_id = _hash(
        {
            "record_type": "AI_BRIEFING_REJECTION",
    ...
    try:
        ...
        if existing:
            if (
                ...
                raise InsightStoreError(
                    "Idempotency key was reused for a different AI rejection."
                )
            return existing, False
        payload = {
            "schema_version": 1,
